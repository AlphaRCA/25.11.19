...
- (void)startRecordAudio {
    ...
    [self prepareWithSampleRate:SAMPLE_RATE];
  ...
  if ([self.audioData length] > chunk_size) {
    ...
    [[SpeechRecognitionService sharedInstance] streamAudioData:self.audioData
                                                ...
                                                withCompletion:^(StreamingRecognizeResponse *response, NSError *error) {
                                                  if (error) {
                                                    NSLog(@"ERROR: %@", error);
                                                    [self.delegate audioControllerDidReceiveError:error];
                                                    [self stopRecordAudio];
                                                  } else if (response) {
                                                    BOOL finished = NO;
                                                    NSLog(@"RESPONSE: %@", response);
                                                      StreamingRecognitionResult *bestResult = nil;
                                                    for (StreamingRecognitionResult *result in response.resultsArray) {
                                                        if (bestResult == nil) {
                                                            bestResult = result;
                                                        }
                                                        if (bestResult.stability < result.stability) {
                                                            bestResult = result;
                                                        }
                                                        if (result.isFinal) {
                                                            finished = YES;
                                                    ...
                                                    [self.delegate audioControllerDidReceiveText:[[[bestResult alternativesArray] lastObject] transcript] isFinal:finished];
//                                                    if (finished) {
//                                                      [self stopAudio:nil];
//                                                    }
                                                  }
